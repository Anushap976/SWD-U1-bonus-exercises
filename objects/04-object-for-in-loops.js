...
for (let food in lunchBox) {
	if (food.startsWith("side")) {
		console.log(lunchBox[food]);
	}
}
...
function itemAtIndex(index){
	...
	for (let i in pantry){
		if (pantry[i][index] !== undefined){
			console.log(`\t${pantry[i][index]}`);
		}
	}
}
...
function printPackageTypeOnly(array, packageType) {
    console.log(`Items that are in ${packageType}: `)
    for (items in array[packageType]) {
        console.log(`- ${array[packageType][items]}`)
    }
}
...
printPackageTypeOnly(pantry, 'bottled')
printPackageTypeOnly(pantry, 'canned')
printPackageTypeOnly(pantry, 'boxed')
printPackageTypeOnly(pantry, 'THIS DOES NOT EXIST')
